...
func (hi *HashIndex) Insert(tid TID, key Field, rid RecID) error {
	...
	current := bucketPage
	for current.IsFull() {
		if current.HasNoOverflowPage() {
			current, err = hi.createAndLinkOverflowPage(tid, bucketNum, current)
			if err != nil {
				return fmt.Errorf("failed to create and link overflow page: %w", err)
			}
			break
		}
		current, err = hi.readOverflowPage(current)
		if err != nil {
			return fmt.Errorf("failed to read overflow page: %w", err)
		}
	}

	if err := current.AddEntry(entry); err != nil {
		return fmt.Errorf("failed to add entry: %w", err)
	}

	return hi.markAndWritePage(current, tid)
}
...
//   - pi: Page to link the new overflow page to
//
// Returns the newly created overflow page or error on failure.
func (hi *HashIndex) createAndLinkOverflowPage(tid TID, bucketNum int, p *HashPage) (*HashPage, error) {
	op, err := hi.file.AllocateOverflowPage(tid, bucketNum)
	if err != nil {
		return nil, fmt.Errorf("failed to allocate overflow page: %w", err)
	}

	p.SetOverflowPage(op.PageNo())
	if err := hi.markAndWritePage(p, tid); err != nil {
...
func (hi *HashIndex) readOverflowPage(p *HashPage) (*HashPage, error) {
	...
	op := p.GetOverflowPageNumber()
	if op >= hi.file.NumPages() {
		return nil, fmt.Errorf("invalid overflow page number %d (max: %d)",
			op, hi.file.NumPages())
	}

	overflowPageID := hash.NewHashPageID(hi.file.GetID(), op)
	page, err := hi.file.ReadPage(overflowPageID)
	if err != nil {
		return nil, fmt.Errorf("failed to read overflow page %d: %w", op, err)
	}
...
func (hi *HashIndex) traverseOverflowChain(startPage *HashPage, f func(*HashPage) error) error {
	var err error
	current := startPage
	visited := make(map[int]bool)

	for current != nil && len(visited) < maxOverflowPages {
		pageNum := current.PageNo()
		if visited[pageNum] {
			break
		}
