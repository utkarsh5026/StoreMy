...
func createTestTable(t *testing.T, ctx *registry.DatabaseContext, tx *transaction.TransactionContext) {
	...
	columns := []schema.ColumnMetadata{
		...
		{Name: "price", FieldType: types.FloatType, Position: 3},
	}

	// Create table schema
	tblSchema, err := schema.NewSchema(0, "test_table", columns)
	if err != nil {
		t.Fatalf("Failed to create schema: %v", err)
	}

	// Create table in catalog
	_, err = ctx.CatalogManager().CreateTable(tx, tblSchema)
	if err != nil {
		t.Fatalf("Failed to create test table: %v", err)
	}

	// Register cleanup to close table file
	testutil.CleanupTable(t, ctx.CatalogManager(), "test_table", tx.ID)
}
