...
func NewTransactionContext(tid *transaction.TransactionID) *TransactionContext {
...
func (tc *TransactionContext) UpdateLSN(lsn log.LSN) {
	tc.mutex.Lock()
	defer tc.mutex.Unlock()
...
func (tc *TransactionContext) GetLastLSN() log.LSN {
	tc.mutex.RLock()
	defer tc.mutex.RUnlock()
	return tc.lastLSN
}
...
func (tc *TransactionContext) GetFirstLSN() log.LSN {
	tc.mutex.RLock()
	defer tc.mutex.RUnlock()
	return tc.firstLSN
}
...
type TransactionRegistry struct {
	contexts map[*transaction.TransactionID]*TransactionContext
	mutex    sync.RWMutex
	wal      *log.WAL
}
...
func NewTransactionRegistry(wal *log.WAL) *TransactionRegistry {
	return &TransactionRegistry{
		contexts: make(map[*transaction.TransactionID]*TransactionContext),
		wal:      wal,
	}
}
...
func (tr *TransactionRegistry) Get(tid *transaction.TransactionID) (*TransactionContext, error) {
	tr.mutex.RLock()
...
func (tr *TransactionRegistry) GetOrCreate(tid *transaction.TransactionID) *TransactionContext {
	tr.mutex.Lock()
	...
	// Create new context for this transaction ID
	ctx = NewTransactionContext(tid)
	tr.contexts[tid] = ctx
	return ctx
}
...
func (tr *TransactionRegistry) Remove(tid *transaction.TransactionID) {
	tr.mutex.Lock()
	defer tr.mutex.Unlock()
...
func (tr *TransactionRegistry) GetAllTransactionIDs() []*transaction.TransactionID {
	tr.mutex.RLock()
	defer tr.mutex.RUnlock()

	tids := make([]*transaction.TransactionID, 0, len(tr.contexts))
