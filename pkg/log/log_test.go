...
func TestNewLogRecord_BeginRecord(t *testing.T) {
	...
	if record.PrevLSN != FirstLSN {
		t.Errorf("expected PrevLSN to be FirstLSN (0), got %d", record.PrevLSN)
	}
}
...
func TestNewLogRecord_NilTransaction(t *testing.T) {
	...
	record := NewLogRecord(CheckpointBegin, nil, nil, nil, nil, FirstLSN)
...
func TestTransactionLogInfo_EmptyTransaction(t *testing.T) {
	info := &TransactionLogInfo{
		FirstLSN:    FirstLSN,
		LastLSN:     FirstLSN,
		UndoNextLSN: FirstLSN,
	}

	if info.FirstLSN != FirstLSN {
		t.Errorf("expected FirstLSN to be FirstLSN, got %d", info.FirstLSN)
	}

	if info.LastLSN != FirstLSN {
		t.Errorf("expected LastLSN to be FirstLSN, got %d", info.LastLSN)
	}
}
