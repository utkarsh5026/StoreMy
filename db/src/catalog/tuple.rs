...
impl_from_reader!(String, "string or null",
    ...
    Value::Dyn(DynValue::Varchar(s) | DynValue::Text(s)) => Some(s.clone())
);
...
impl_from_reader!(u64, "uint64",
    Value::Fixed(FixedValue::Uint64(n)) => Some(*n),
    Value::Fixed(FixedValue::Int64(n)) => Some((*n).cast_unsigned()),
    Value::Fixed(FixedValue::Uint32(n)) => Some(u64::from(*n))
);

impl_from_reader!(u32, "uint32", Value::Fixed(FixedValue::Uint32(n)) => Some(*n));

// `Value::Int32` is widened losslessly to `i64`.
impl_from_reader!(i64, "i64",
    Value::Fixed(FixedValue::Int32(n)) => Some(i64::from(*n)),
    Value::Fixed(FixedValue::Int64(n)) => Some(*n)
);

impl_from_reader!(i32, "i32", Value::Fixed(FixedValue::Int32(n)) => Some(*n));
impl_from_reader!(bool, "bool", Value::Fixed(FixedValue::Bool(b)) => Some(*b));
...
mod tests {
    ...
    fn test_extract_wrong_type_error_message_contains_field_number() {
        ...
        let err = reader
            .extract("i32", |v| match v {
                Value::Fixed(FixedValue::Int32(n)) => Some(*n),
                _ => None,
            })
            .unwrap_err();
    ...
    fn test_string_from_string_value() {
        let t = tuple(vec![Value::varchar("hello".into())]);
