...
use crate::database::{Database, DatabaseConfig, DatabaseOpenError};
...
pub enum RegistryError {
    ...
    InvalidName(String),
    /// Directory scan / creation failures (not database-boot failures).
    #[error("I/O error: {0}")]
    Io(#[from] std::io::Error),
    /// Any failure that occurs while booting a database.
    #[error(transparent)]
...
impl DatabaseRegistry {
    ...
    pub fn initialize(root: impl AsRef<Path>) -> Result<Self, RegistryError> {
        ...
        for entry in std::fs::read_dir(&root)? {
            ...
            match Database::open(&entry.path(), &DatabaseConfig::default()) {
                Ok(db) => {
    ...
    pub fn create(&self, name: &str) -> Result<(), RegistryError> {
        ...
        let db = Database::open(&db_dir, &DatabaseConfig::default())?;
